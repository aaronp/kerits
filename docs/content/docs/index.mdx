...
| `Said`      | SAID computation and verification                          |
...
```ts
// Namespace style
import { Kel, Said, Cesr } from '@kerits/core';
const said = Said.encodeSAID(event);
const encoded = Cesr.encodeKey(rawPublicKey);

// Flat style
import { encodeSAID, encodeKey } from '@kerits/core';
```
